...
- As reviewer recommend not to use rarefying so i only use TSS to normalize it 



```{r normalization}
ps.1.tss <- transform_sample_counts(ps.1, function(x) x/sum(x) )


mischanthus <- prune_samples((sample_data(ps.1.tss))$Plant =="Miscanthus", ps.1.tss)
mis.noBulk <- prune_samples(sample_data(mischanthus)$Soil =="Rizosphere", mischanthus)#432 samples

sample_data(mis.noBulk)$Year <- as.factor(sample_data(mis.noBulk)$Year)

sample_data(mis.noBulk)$Stand_age <- ifelse(sample_data(mis.noBulk)$Year == "2015", "4",
                                           ifelse(sample_data(mis.noBulk)$Year =="2016", "3",
                                                  ifelse(sample_data(mis.noBulk)$Year == "2017", "2","NA")))
...
saveRDS(mis.noBulk, "mis_noBulk_noRarefing.rds")
```
...
ggsave(other_venn, file="VennDiagram_20200625.pdf")

```

- The numbers in the Venn diagram are different from rarefying one, which makes senses. 
-- 1  Rarefying removed 16 samples, which will contribute to the ASVs numbers here 
-- 2 Rarefying may move reads with greated numbers

# But, does non-rarefying change the Fig3?
...
shared.151617.phylo <- subset_taxa(mis.noBulk, taxa_names(mis.noBulk) %in% shared.151617)
...
ggsave(shared.151617.phylo.bar, file="barofmicrobialCommunityofMischanthus_shared151617_nonrarefying.pdf",units="in", width=9, height=9)

unique2015.phy.relative <- subset_taxa(mis.noBulk, taxa_names(mis.noBulk) %in% unique.2015)
...
unique2016.phy.relative <- subset_taxa(mis.noBulk, taxa_names(mis.noBulk) %in% unique.2016)
...
unique2017.phy.relative <- subset_taxa(mis.noBulk, taxa_names(mis.noBulk) %in% unique.2017)
...
ggsave(shared.unique.taxa.plot, file="shared_unique_taxa_figre3_nonRarefying.pdf", units="in", width=9, height=9)


```

- Trends are same as what we saw with rarefying data, even though the number may the not exact same. 

# does non_rarefying affects the PERMANOVA analysis?
```{r}
...
as.factor(sample_data(mis.noBulk)$Year)       2    11.574  5.7871  32.552 0.09528  0.001 ***
as.factor(sample_data(mis.noBulk)$Nitrogen)   2     1.762  0.8811   4.956 0.01451  0.001 ***
as.factor(sample_data(mis.noBulk)$Plot)      31    37.131  1.1978   6.737 0.30565  0.001 ***
as.factor(sample_data(mis.noBulk)$Days)       4     1.324  0.3311   1.862 0.01090  0.011 *  

-  largest observed variation to affect this diversity was the plot of sample origination:  Same as rarefing data
- Second largest variation was still Year (Stand age) followed by fertilization rate:  same as rarefying data.
- Days also affect the community structure siginificantly: same as rarefying data.

# Prevalence first before do the venn diagram
```{r prevalence_filted_vennDiagram}
# function make same fig2 venn diagram
Venn_function <- function(mis.noBulk){
  mis.noBulk.2015 <- prune_samples(sample_data(mis.noBulk)$Year =="2015", mis.noBulk)
  no.zero.mis.noBulk.2015 <- prune_taxa(taxa_sums(mis.noBulk.2015) > 0, mis.noBulk.2015)
  list.2015 <- rownames(otu_table(no.zero.mis.noBulk.2015))
...
mis.noBulk.p001 <- filterTaxaByPrevolence(mis.noBulk, 0.01)
mis.noBulk.p001.venn <- Venn_function(mis.noBulk.p001)
ggsave(mis.noBulk.p001.venn, file="venn_prevalence001.pdf")
